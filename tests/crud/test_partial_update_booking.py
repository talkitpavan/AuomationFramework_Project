...
@pytest.mark.smoke
@allure.title("TC_04 Partial Update Booking ")
def test_partial_update_booking_by_id(create_booking_id,create_token_id):
    # base_url = "https://restful-booker.herokuapp.com"
    # base_path = "/booking"
    # url = base_url + base_path +"/"+ str(create_booking_id)
    # headers = {'Content-Type': 'application/json'}
    ar = Api_Request()
    response_data = ar.patch_request(url_update_partial_booking(create_booking_id), auth=None, headers=auth_headers(create_token_id), payload=partial_update_booking_payload())
